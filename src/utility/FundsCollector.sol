...
abstract contract FundsCollector {
    ...
    error PartialWithdrawlsNotAllowed();
    error TokenNotSet();
    error TokenTransferFailed();
    error CannotHaveAmountWithEntireBalanceOption();
    error CannotWithdrawMoreThanBalance(
        uint256 balance,
    ...
    function withdrawAmount(uint256 amount) external {
        if (!allowPartialWithdrawls && amount>0 && (amount < balances[msg.sender]))
            revert PartialWithdrawlsNotAllowed();
    ...
    function withdraw(bool entireBalance, uint256 amount) internal {
        ...
        //     revert CannotHaveAmountWithEntireBalanceOption();
        uint256 currentBalance = balances[msg.sender];
        if (amount > currentBalance)
            revert CannotWithdrawMoreThanBalance(currentBalance, amount);
        if (!entireBalance && amount == 0) {
            emit AttemptToWithdrawZeroAmount();
            return;
        }

        if (entireBalance) amount = currentBalance;
    ...
    ///@dev called from a super class, where the payment is collected
    function _collectPayment(
        ...
        if (paymentToken == address(0)) {
            ...
            bool result = IERC20(paymentToken).transferFrom(
                from,
                address(this),
                amount
            );
            if (!result) revert TokenTransferFailed();
            if (to != address(0)) balances[to] += amount;
        }
    }
}
